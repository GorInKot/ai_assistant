...
PARTICIPANT_QUERY_RE = re.compile(
    # \bроль — граница слова обязательна, иначе «контроль» (строительный
    # контроль — ключевой термин продукта) ложно распознаётся как вопрос про
    # участников и вместо шагов процесса возвращается список ролей.
    r"(участник\w*|актор\w*|\bрол[ьиея]\w*|заявител\w*|заказчик\w*|исполнител\w*|participant\w*|actor\w*|role\w*)",
    re.IGNORECASE,
)
